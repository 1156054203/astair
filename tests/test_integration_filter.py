...
class CallOutputTest(unittest.TestCase):
    ...
    def test_filter_default_taps(self):
        ...
        cytosine_modification_finder(current + '/test_data/small_real_taps_lambda_mCtoT_high_CpH_filtered.bam', None, None, current + '/test_data/lambda_phage.fa', 'all', False, False, 13, None, 'directional', 'mCtoT', 0, 0, True, False, True, True, 250, None, 1, current + '/test_data/', False, False, False, 0)
        data_generated = list()
    ...
    def test_filter_default_wgbs(self):
        ...
        cytosine_modification_finder(current + '/test_data/small_real_wgbs_lambda_CtoT_high_CpH_filtered.bam', None, None, current + '/test_data/lambda_phage.fa', 'all', False, False, 13, None, 'directional', 'CtoT', 0, 0, True, False, True, True, 250, None, 1, current + '/test_data/', False, False, False, 0)
        data_generated = list()
    ...
    def test_filter_default_wgbs_gzip(self):
        ...
        if path.isfile(current + '/test_data/lambda_phage_.fa'):
            subprocess.Popen('gzip {}'.format(current + '/test_data/lambda_phage_.fa'), shell=True)
        cytosine_modification_finder(current + '/test_data/small_real_wgbs_lambda_CtoT_high_CpH_filtered.bam', None, None, current + '/test_data/lambda_phage_.fa.gz', 'all', False, False, 13, None, 'directional', 'CtoT', 0, 0, True, False, True, True, 250, None, 1, current + '/test_data/', False, False, False, 0)
        data_generated = list()
        with open(current + '/test_data/small_real_wgbs_lambda_CtoT_high_CpH_filtered_CtoT_all.stats','r') as call_file:
            mod_reader = csv.reader(call_file, delimiter='\t', lineterminator='\n')
            for row in mod_reader:
