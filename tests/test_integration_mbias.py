...
class MbiasOutputTest(unittest.TestCase):
    ...
    def test_mbias_default_taps_SE(self):
        ...
        Mbias_plotting(current + '/test_data/lambda_phage.fa', current + '/test_data/small_real_taps_lambda_mCtoT_SE.bam', current + '/test_data/', 75, 'mCtoT', True, False, ['teal', 'gray', 'maroon'], 1, None)
        data_generated = list()
    ...
    def test_mbias_default_wgbs(self):
        ...
        Mbias_plotting(current + '/test_data/lambda_phage.fa', current + '/test_data/small_real_wgbs_lambda_CtoT.bam', current + '/test_data/', 80, 'CtoT', False, False, ['teal', 'gray', 'maroon'], 1, None)
        data_generated = list()
        with open(current + '/test_data/small_real_wgbs_lambda_CtoT_Mbias.txt','r') as call_file:
            mod_reader = csv.reader(call_file, delimiter='\t', lineterminator='\n')
            for row in mod_reader:
                data_generated.append(tuple((row)[0:2]))
        self.assertEqual(data_generated[-10:-1], [('71', '100.0'), ('72', '100.0'), ('73', '100.0'), ('74', '100.0'), ('75', '50.0'), ('76', '100.0'), ('77', '100.0'), ('78', '100.0'), ('79', '100.0')])
