...
@click.option('reference', '--reference', '-f', required=True, help='Reference DNA sequence in FASTA format used for aligning the sequencing reads.')
...
@click.option('output_format', '--output_format', '-O', required=False, default='cram', type=click.Choice(['cram', 'bam']), help='Specify output format, possible options are cram and bam. The default is cram.')
...
@click.option('skip_mate_rescue', '--skip_mate_rescue', '-s', is_flag=True, required=False, help='NB: Does not recommend unless necessary: skips mate rescue, see BWA manual. If set, orphan reads (paired reads that are not in a proper pair) will be generated. Ensure ignore_orphans in the caller is set to False.')
@click.option('skip_pairing', '--skip_pairing', '-P', is_flag=True, required=False, help='NB: Does not recommend unless necessary: skips read pairing, but does rescue mates unless mate_skipping is also performed, see BWA manual. If set, orphan reads (paired reads that are not in a proper pair) will be generated. Ensure ignore_orphans in the caller is set to False.')
...
def run_alignment(fq1, fq2, reference, bwa_path, samtools_path, directory, method, output_format, minimum_mapping_quality, keep_unmapped, N_threads, minimum_seed_length, band_width, dropoff,
    ...
    if method == 'mCtoT':
        alignment_command = '{} {} -t {} -k {} -w {} -d {} -r {} -y {} -c {} -D {} -W {} -m {} {} ' \
                            ' {} -A {} -B {} -O {} -E {} -L {} -U {} {} {} {} {} | {} view {} -T {} -q {} {} ' \
                            '| {} sort -@ {} > {}'.format(use_bwa, 'mem', N_threads, minimum_seed_length, band_width, dropoff,
                 internal_seeds, reseeding_occurence, N_skip_seeds, drop_chains, discard_chains, N_mate_rescues, skip_mate_rescue, skip_pairing, match_score, mismatch_penalty,
                 gap_open_penalty, gap_extension_penalty, end_clipping_penalty, unpaired_penalty, read_type,
                   reference, fq1, fq2, use_samtools, format, reference, minimum_mapping_quality,
        ...
        alignment_command = 'python2 {} -t {} --reference {} {} {} | {} view {} -T {} -q {} {} | {} sort -@ {} > {}'.\
            format(use_bwa, N_threads, reference, fq1, fq2, use_samtools, format, reference, minimum_mapping_quality,
                   aligned_string, use_samtools, N_threads, os.path.join(directory + name + "." + output_format))
    try:
