...
@click.option('start_clip', '--start_clip', '-scl', required=False, type=int, default=0, help='Set the length of the bases in the start (5 prime) of the reads that will not be used for calling. (Default 0).')
@click.option('end_clip', '--end_clip', '-ecl', required=False, type=int, default=0, help='Set the length of the bases in the end (3 prime) of the reads that will not be used for calling. (Default 0).')
        ...
        try:
            if ((reads.reference_name, reads.pos, reads.pos + 1) in modification_information_per_position) or (possible_mods is not None and (reads.reference_name, reads.pos, reads.pos + 1) in possible_mods):
                ...
                for pileup, seq, pos in zip_longest(reads.pileups, sequences, reads.get_query_positions(), fillvalue='BLANK'):
                    if (start_clip == 0 and end_clip == 0) or (pileup.alignment.flag in [83, 99] and pos > start_clip and pos < (pileup.alignment.rlen - end_clip)) or (pileup.alignment.flag in [147, 163] and pos > end_clip and pos < (pileup.alignment.rlen - start_clip)):
