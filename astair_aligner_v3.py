...
@click.option('output_format', '--output', '-o', required=False, default='cram', type=click.Choice(['cram', 'bam']), help='Specify output format, possible options are cram and bam. The default is cram.')
...
@click.option('N_threads', '--N_threads', '-t', default=10, required=True, type=int, help='The number of threads to spawn (the default value is 10).')
...
@click.option('skip_mate_rescue', '--skip_mate_rescue', '-s', is_flag=True, required=False, help='Skips mate rescue, see BWA manual.')
@click.option('skip_pairing', '--skip_pairing', '-P', is_flag=True, required=False, help='Skips read pairing, but does rescue mates unless mate_skipping is also performed, see BWA manual.')
...
@click.option('gap_open_penalty', '--gap_open_penalty', '-O', default=[6,6], type=list, required=False, help='The gap open penalties for deletions and insertions, see BWA manual (the default value is 6,6).')
@click.option('gap_extension_penalty', '--gap_extension_penalty', '-E', default=[1,1], type=list, required=False, help='The gap extension penalty with a cost size calculated as {-O} + {-E}*k, see BWA manual (the default value is 1,1).')
@click.option('end_clipping_penalty', '--end_clipping_penalty', '-L', default=[5,5], type=list, required=False, help='The penalty for 5-prime- and 3-prime-end clipping, see BWA manual (the default value is 5,5).')
...
@click.option('read_type', '--N_threads', '-x', default=None, type=click.Choice([None, 'pacbio', 'ont2d', 'intractg']), required=False, help='Changes multiple parameters unless overridden, see BWA manual (the default value is None).')
def aligner_exec(fq1, fq2, fasta_file, bwa_path, samtools_path, directory, method, output_format, minimum_mapping_quality, keep_unmapped, N_threads, minimum_seed_length, band_width, dropoff,
    ...
    run_alignment(fq1, fq2, fasta_file, bwa_path, samtools_path, directory, method, output_format, minimum_mapping_quality, keep_unmapped, N_threads, minimum_seed_length, band_width, dropoff,
                 internal_seeds, reseeding_occurence, N_skip_seeds, drop_chains, discard_chains, N_mate_rescues, skip_mate_rescue, skip_pairing, match_score, mismatch_penalty,
                 gap_open_penalty, gap_extension_penalty, end_clipping_penalty, unpaired_penalty, read_type)
...
def check_index(use_bwa, fasta_file, method):
    """Checks if the provided reference is indexed, and creates an index if one is not found."""
    if (os.path.isfile(fasta_file + '.bwt') == False and method == 'CmtoT') \
            or (os.path.isfile(fasta_file + '.bwameth.c2t') == False and method == 'CtoT'):
        build_command = '{} index {}'.format(use_bwa, fasta_file)
        index_fasta = subprocess.Popen(build_command, shell=True)
        index_fasta.wait()


def run_alignment(fq1, fq2, fasta_file, bwa_path, samtools_path, directory, method, output_format, minimum_mapping_quality, keep_unmapped, N_threads, minimum_seed_length, band_width, dropoff,
                 internal_seeds, reseeding_occurence, N_skip_seeds, drop_chains, discard_chains, N_mate_rescues, skip_mate_rescue, skip_pairing, match_score, mismatch_penalty,
                 gap_open_penalty, gap_extension_penalty, end_clipping_penalty, unpaired_penalty, read_type):
    ...
    if output_format == 'cram':
        ...
        format = '-hb'
    check_index(use_bwa, fasta_file, method)
    if method == 'CmtoT':
        alignment_command = '{} {} -t {} -k {} -w {} -d {} -r {} -y {} -c {} -D {} -W {} -m {} -S {} ' \
                            '-P {} -A {} -B {} -O {} -E {} -L {} -U {} -x {} {} {} {} | {} view {} -T {} -q {} {} ' \
                            '| {} sort -@ {} > {}'. \
            format(use_bwa, 'mem', N_threads, minimum_seed_length, band_width, dropoff,
                 internal_seeds, reseeding_occurence, N_skip_seeds, drop_chains, discard_chains, N_mate_rescues, skip_mate_rescue, skip_pairing, match_score, mismatch_penalty,
                 gap_open_penalty, gap_extension_penalty, end_clipping_penalty, unpaired_penalty, read_type,
                   fasta_file, fq1, fq2, use_samtools, format, fasta_file, minimum_mapping_quality,
